  ...
  return (
    ...
    >
      <div className="flex flex-col items-center gap-1 pt-0.5">
        <button
          onClick={onUpvote}
          className={`text-[11px] leading-none ${hasUpvoted ? 'text-brand-red' : 'text-brand-ink-soft'}`}
        >
          ▲
        </button>
        <span className="text-[15px] font-semibold text-brand-navy tabular-nums">{post.upvotes?.length || 0}</span>
        <span className="text-[11px] leading-none text-brand-ink-soft">▼</span>
      </div>
      ...
      <div className="px-5 sm:px-8 lg:px-12 py-7 sm:py-8 lg:py-9 max-w-[900px] flex flex-col gap-6">
        <div className="grid grid-cols-[48px_1fr] sm:grid-cols-[64px_1fr] gap-4">
          <div className="flex flex-col items-center gap-1">
            <button onClick={onUpvote} className={`text-[12px] ${hasUpvoted ? 'text-brand-red' : 'text-brand-ink-soft'}`}>
              ▲
            </button>
